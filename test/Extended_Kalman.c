...
//	float_j_t pi = atan2(a_y,a_z);//*180/3.14;
	...
	z_k[0][0] = atan2(-a_y,-a_z);
	if(z_k[0][0]<0){
		z_k[0][0] = atan2(-a_y,-a_z)+3.14;	
	}else{
		z_k[0][0] = atan2(-a_y,-a_z)-3.14;	

	}
	z_k[1][0] = -atan2(-a_x,a_z);
...
#define Gravity -9.8
		...
		estimate_theta = (float_j_t)asin(val2);
		//estimate_theta = (float_j_t)asin((double)-a_x/Gravity)*180.0/3.14; //rad to degree 
...
//		theta += 0.0001;
	...
	C_inv[0][0] = 1*dt;
	C_inv[0][1] = sin(pi)*tan(theta)*dt; 
	C_inv[0][2] = cos(pi)*tan(theta)*dt;
	C_inv[1][0] = 0;
	C_inv[1][1] = cos(pi)*dt;
	C_inv[1][2] = -sin(pi)*dt;
	C_inv[2][0] = 0;
	C_inv[2][1] = (sin(pi)/cos(theta))*dt;
	C_inv[2][2] = (cos(pi)/cos(theta))*dt;
	...
	/* A_k*/
	float_j_t A[3][3]={0,};
	A[0][0] = 1 + (sin(theta)*(omega[1][0]*cos(pi)-omega[2][0]*sin(pi)) / cos(theta))*dt;
	A[0][1] = ((omega[1][0]*sin(pi)+omega[2][0]*cos(pi)) / (cos(theta)*cos(theta)))*dt;
	A[0][2] = 0;

	A[1][0] = (-omega[1][0]*sin(pi)-omega[2][0]*cos(pi))*dt;
	A[1][1] = 1;
	A[1][2] = 0;

	A[2][0] = ((omega[1][0]*cos(pi)-omega[2][0]*sin(pi)) / cos(theta))*dt;
	A[2][1] = (sin(theta)*(omega[1][0]*sin(pi)+omega[2][0]*cos(pi)) / (cos(theta)*cos(theta)))*dt;
	A[2][2] = 1;
	...
	norm_a=sqrt(sen->accX*sen->accX + sen->accY*sen->accY + sen->accZ*sen->accZ);
//		printf("norm_a = %f\n",norm_a);
	float_j_t R[3][3]={0,};
	if((norm_a<9.9 )&&(norm_a>9.7)){
		R[0][0]=1;
		R[1][1]=1;
	}else{
		R[0][0]=1000000;
...
//			printf("example!\ninput accX,Y,Z, gyroX,Y,Z, dt 7Parameter\n");
			...
			sensor.gyroX=a*3.14/180;
			sensor.gyroY=b*3.14/180;
			sensor.gyroZ=c*3.14/180;
