...
#if 0
    uint8_t buf[6];
	i2c_read(MPU6050_DEV, buf, 6, MPU6050_RA_GYRO_XOUT_H);
...
int device_init(int fd)
{
	...
	i2c_simpleWrite(fd, MPU6050_RA_SMPLRT_DIV, MPU_SAMPLE_RATE);
	//Disable FSync, 256Hz DLPF
	i2c_simpleWrite(fd, MPU6050_RA_CONFIG, (DLPF_CFG));

	//Disable gyro self tests, scale of 250 degrees/s
	i2c_simpleWrite(fd, MPU6050_RA_GYRO_CONFIG, (GYRO_FS_SEL<<3));
	//Disable accel self tests, scale of +-2g, no DHPF
	i2c_simpleWrite(fd, MPU6050_RA_ACCEL_CONFIG, (ACC_FS_SEL<<3));//  +- 16G 
...
int mpu6050_init()
{
	...
	i2c_open();
	i2c_setSlave(MPU6050_DEV);
	device_init(fd_sensor);
	return 0;
}
